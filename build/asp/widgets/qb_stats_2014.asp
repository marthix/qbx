<div class="box box-warning">
	<div class="box-header with-border">
		...
		<h3 class="box-title">2014 Season Statistics</h3>
...
<%
		...
		sqlGetPassingPercentage = "SELECT qb_ID, (SUM(tracker_PassingCompletions) / SUM(tracker_PassingAttempts))* 100 AS PaComp FROM samelevel.qbx_tracker WHERE game_ID IN (SELECT game_ID FROM samelevel.qbx_games WHERE game_Year = 2014) GROUP BY qb_ID ORDER BY PaComp DESC"
		...
		sqlGetPassingYards = "SELECT qb_ID, SUM(tracker_PassingYards) AS PaYDS FROM samelevel.qbx_tracker WHERE game_ID IN (SELECT game_ID FROM samelevel.qbx_games WHERE game_Year = 2014) GROUP BY qb_ID ORDER BY PaYDS DESC"
		...
		sqlGetPassingTD = "SELECT qb_ID, SUM(tracker_PassingTouchdowns) AS PaTD FROM samelevel.qbx_tracker WHERE game_ID IN (SELECT game_ID FROM samelevel.qbx_games WHERE game_Year = 2014) GROUP BY qb_ID ORDER BY PaTD DESC"
		...
		sqlGetRushingTD = "SELECT qb_ID, SUM(tracker_RushingTouchdowns) AS RuTD FROM samelevel.qbx_tracker WHERE game_ID IN (SELECT game_ID FROM samelevel.qbx_games WHERE game_Year = 2014) GROUP BY qb_ID ORDER BY RuTD DESC"
		...
		sqlGetRushingYDS = "SELECT qb_ID, SUM(tracker_RushingYards) AS RuYDS FROM samelevel.qbx_tracker WHERE game_ID IN (SELECT game_ID FROM samelevel.qbx_games WHERE game_Year = 2014) GROUP BY qb_ID ORDER BY RuYDS DESC"
